...
export const generateFittingImage = async (
  ...
  const promptText = `첫 번째 이미지에 있는 사람에게 ${imageDescriptions.join('와 ')}를 입혀주세요. 옷의 질감, 주름, 그리고 그림자를 사실적으로 표현하여 마치 실제로 입은 것처럼 자연스럽게 보여야 합니다. 사람의 포즈, 얼굴, 그리고 배경은 첫 번째 이미지를 기준으로 변경하지 말고 그대로 유지해주세요.`;
